...
function buildPrintDocument({title,body,reference='',date='',settings,signature=true,hideReference=false}){
  ...
  return `<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title||'Document')}</title><style>${printBaseStyles()}</style></head><body class="${bodyClass}">${officialHeaderHtml(settings,{reference,hideReference})}${body}${signature?officialSignatureHtml(settings,date):''}${officialFooterHtml(settings)}</body></html>`;
}
...
async function printRecord(record){
  try{
    ...
    if(moduleKey==='absences'){
      ...
      const objectLines=objectText?`<div class="convocation-writing-line">${esc(objectText)}</div>`:'<div class="convocation-writing-line"></div>';
      title='CONVOCATION';
      body=`<div class="document-title">CONVOCATION</div><div class="official-body convocation-body"><div class="convocation-row"><strong>M./Mme/Mlle :</strong> <span class="convocation-dotted convocation-long">${esc(`${civ} ${record.title}`.trim())}</span></div><div class="convocation-row convocation-inline"><strong>Profession :</strong><span class="convocation-dotted grow">${esc(displayValue(d.profession))}</span><strong>Domicile :</strong><span class="convocation-dotted grow">${esc(displayValue(d.domicile))}</span><span>est prié(e) de bien vouloir se présenter au <strong>${esc(structure)}</strong>, le</span><span class="convocation-dotted">${esc(presentDate)}</span><span>à</span><span class="convocation-dotted">${esc(presentTime)}</span><span>heures, muni d’une pièce d’identité.</span></div><div class="convocation-row"><div class="convocation-section-title">Objet de la convocation :</div>${objectLines}<div class="convocation-writing-line"></div><div class="convocation-writing-line"></div></div><div class="convocation-row"><div class="convocation-section-title">Personne à voir :</div><div class="convocation-writing-line">${esc(personText)}</div><div class="convocation-writing-line"></div></div><p class="convocation-final">Votre présence à la date et à l’heure indiquée est nécessaire.</p></div>`;

import {api,esc,loadSession,withButtonLock,professionalAlert,professionalDialog} from './common.js?v=1.67';
...
function bindResponsiveNav(){
  ...
  nav.querySelectorAll('.nav-group>button').forEach(btn=>btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();const group=btn.closest('.nav-group');const willOpen=!group.classList.contains('is-open');closeGroups(group);group.classList.toggle('is-open',willOpen)}));
  nav.querySelectorAll('a').forEach(a=>a.addEventListener('click',()=>{if(mobile())closeAll();else closeGroups()}));document.addEventListener('click',e=>{if(!e.target.closest('.nav-group'))closeGroups()});addEventListener('resize',()=>{if(!mobile())closeMobile()});
}
